...
def replace_method(the_class, name, replacement, ignore_missing=False):
    # NOTE: This is like the definition of the wrapper that translate to python return conventions
    ...
    # if name == "add_and_search":
    #     print(type(the_class), the_class.__name__)
    #     print("Original method", orig_method, orig_method.__name__)
    #     print("Replacement method", replacement, replacement.__name__)
...
def handle_Index(the_class):
    ...
    def replacement_add_and_search(self, x):
        n, d = x.shape
        assert d == self.d
        ...
        self.add_and_search_c(n, swig_ptr(x), swig_ptr(distances), swig_ptr(labels))
    ...
    def replacement_search_and_return_centroids(self, x, k, *, params=None, D=None, I=None, C=None):
        nprobe = min(self.nlist, params.nprobe if params else self.nprobe) # use the identical way as the IndexIVF to obtain the nprobe
        n, d = x.shape
        x = np.ascontiguousarray(x, dtype='float32')
        assert d == self.d
        ...
        if C is None:
            C = np.empty((n, nprobe), dtype=np.int64)
        else:
            assert C.shape == (n, nprobe)

        nprobe = self.search_and_return_centroids_c(n, swig_ptr(x), k, swig_ptr(D), swig_ptr(I), swig_ptr(C), params)
        return D, I, C, nprobe
    ...
    replace_method(the_class, 'add_and_search', replacement_add_and_search, ignore_missing=True)
    replace_method(the_class, 'assign', replacement_assign)
    replace_method(the_class, 'train', replacement_train)
    replace_method(the_class, 'search', replacement_search)
    replace_method(the_class, 'search_and_return_centroids', replacement_search_and_return_centroids, ignore_missing=True)
