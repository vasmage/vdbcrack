...
struct IndexIVF : Index, IndexIVFInterface {
    ...
    /// same as add(idx_t n, const float* x) but also return the centroid ids and distances
    void add_and_search(idx_t n, const float* x, float* distances, idx_t* labels);
    ...
    size_t search_and_return_centroids(
            idx_t n,
            const float* x,
            idx_t k,
            float* distances,
