...
void IndexIVF::add_and_search(idx_t n, const float* x, float* distances, idx_t* labels) {
    quantizer->search(n, x, 1, distances, labels); // set k = 1 to return the nearest centroid
    add_core(n, x, nullptr, labels);
}
...
void IndexIVF::search(
    ...
    search_and_return_centroids(n, x, k, distances, labels, nullptr, params_in);
}

size_t IndexIVF::search_and_return_centroids(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
        idx_t* labels,
        idx_t* idx, // used to return the indices of the centroids
        const SearchParameters* params_in) const {
    ...
    if (idx == nullptr) {
        idx_ptr.reset(new idx_t[n * nprobe]);
        idx = idx_ptr.get();
    }
    ...
    #pragma omp parallel for if (nt > 1)
        ...
        // all)
        sub_search_func(n, x, distances, labels, idx, &indexIVF_stats);
    }
