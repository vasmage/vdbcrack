...
void Clustering::train_encoded(
    ...
    // x is a pointer to the data x_in which is of 
    ...
    if (nx == k) {
        ...
        index.reset();
        index.add(k, centroids.data()); // a flat index is used to store centroids
        return;
    }
    ...
    for (int redo = 0; redo < nredo; redo++) {
        ...
        // add centroids at every iteration as they're updated...
        ...
        for (int i = 0; i < niter; i++) {
            ...
            if (!codec) {
                // CHEF NOTE: for all training points (x) you search the centroid index to get assignments 'assign'
                // CHEF SOS: if nx == total poitns in centroids, you can get assignments directly here rather than outside
                // dynamic cast of uint8 codes xin, to float* in order to work with them:
                // CHEF Q: .search() know how to iterate over x which is what?
                //  - CHEF TODO: Can I have custom .search() which can operate over invlists correctly?
                // CHEF NOTE TODO:
                //    - .train() in normal index is called once at the start before invlists is created or the quantizer etc.
                //    - if I crack, I call it again an again, so the overheads of creating "xin" and "x" which is a contigoous array requires copies
                //    - Q: can I operate directly over invlists, as if it's contigouus (losing a bit on simd whatever), and then only .reorg() based on assigments at the end of the local kmeans 
