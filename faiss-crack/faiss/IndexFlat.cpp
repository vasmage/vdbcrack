...
// CHEF TODO :
//      - for local KMeans: 
//      - if you want to modify this to work over an invlist x cant be const (n*d)
//      - NOTE: probably create another version of ::search() where it can see that x is view over ArrayInvertedLists
//              - and it can iterate over each of them to get assignments and return them so call .knn() O(invlists) number times
//              - each invlist is contigoous 
