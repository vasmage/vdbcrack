...
void ArrayInvertedLists::add_empty_list() {
    // Add empty entries to ids and codes
    nlist++; // Update the number of lists
    ids.resize(nlist);
    codes.resize(nlist);
    // ids.emplace_back(); // Adds an empty vector for IDs
    // codes.emplace_back(); // Adds an empty vector for codes
}
