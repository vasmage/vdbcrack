...
void IndexIVFFlat::add_core(
    ...
    assert(invlists);
    direct_map.check_can_add(xids); // CHEF Q: Is this checking if add_preassigned() can add an id i.e. that it's been removed prior w/ removed_ids()?

    int64_t n_add = 0;

    DirectMapAdd dm_adder(direct_map, n, xids); // CHEF NOTE: this handles direct map adding 
