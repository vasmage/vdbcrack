...
namespace tools {
    bool checkHDF5(const char *filename) {
        // check if the suffix is .hdf5
        std::string filename_str(filename);
        if (filename_str.substr(filename_str.find_last_of(".") + 1) != "hdf5") {
            std::cerr << "The file is not a hdf5 file" << std::endl;
            return false;
        }
        return true;
    }

    void loadHDF5(const char *filename, const char* dataset_name, float ***centers, int *numObjects, int *dimension) {
        std::string filename_str(filename);
        if (!checkHDF5(filename)) {
            exit(1);
        }
        ...
        *numObjects = dims[0];
        *dimension = dims[1];
        std::cout << "Loaded " << *numObjects << " points of dimension " << *dimension << std::endl;
        
        float* centers_raw = (float *)malloc(*numObjects*sizeof(float)*(*dimension));
        dataset.read_raw(centers_raw);
        
        *centers = (float **)malloc(*numObjects*sizeof(float *));
        for(int i=0;i<*numObjects;i++)
        {
            (*centers)[i] = centers_raw + i*(*dimension);
        }

        std::cout << filename  << ": " << dataset_name << " loaded" << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    cout << "Find " << K << "nn" << endl;
    cout << "Queries run: " << target_queries << endl;
    if(K==0)
    {
    ...
    #else
        tools::loadHDF5(argv[optind], "train", &points, &numPoints, &dimension);
        int q_d = 0;
        tools::loadHDF5(argv[optind+1], "test", &queries, &numQueries, &q_d);
        if(dimension != q_d)
        {
            cout << "Dimension mismatch between point and query dataset\n";
            exit(1);
        }
        if(target_queries == 0) {
            target_queries = numQueries;
        }
        // tools::loadData(argv[optind], &points, &numPoints, &dimension);
        // tools::loadQueries(argv[optind+1], &queries, &radius, &numQueries, dimension );
    #endif  

    double* per_query_time = new double[target_queries];
    ofstream myfile;
    ...
    // CHEF NOTE: L2 distance:
        switch (runProcessingMethod)
        {
            ...
            case KNNMediocre:
                ...
                begin = clock();
                for(int q = 0; q < target_queries; q++)
                {
                    i = q % numQueries;
                    ...
                    beginPerQuery = clock();
                    // They seem to operate directly on the points and queries data
                    // They don't need to "add" the points to the nodes, the index is separate, which is an advantage I would say?
                    // But in FAISS you have to .add() points to an index before it is queryable so this is a limitation in our case
                    //      - for the very very small index case, we lose a few ms I guess
                    TwoPQ::knnMediocre(root,queries[i],points,&distances,guide_pq,result,ids,dimension,i);
                    endPerQuery = clock() - beginPerQuery;
                    // tools::showpq(result);
                    // cout << (double)endPerQuery/CLOCKS_PER_SEC << endl;
                    per_query_time[q] = (double)endPerQuery/CLOCKS_PER_SEC;
                    if(q % 10000 == 0) {
                        cout << "Finished " << i << " queries" << endl;
                    }
                }
                end = clock() - begin;
                cout << "Total time " << (double)end/CLOCKS_PER_SEC << endl;

                dataset = dataset.substr(dataset.find_last_of("/\\") + 1, dataset.find_last_of(".") - dataset.find_last_of("/\\") - 1);
                filename = dataset + "_mediocre_per_query_time_Top" + std::to_string(K) + "_threshold_" + std::to_string(CRACKTHRESHOLD) + ".csv";
                myfile.open (filename);
                myfile << "qid,Time," << endl;
                for(int i = 0; i < target_queries; i++) {
                    myfile << i << "," << per_query_time[i] << endl;
                }
                myfile.close();
            ...
            case KNNCrackOnKth:
                ...
                dataset = dataset.substr(dataset.find_last_of("/\\") + 1, dataset.find_last_of(".") - dataset.find_last_of("/\\") - 1);
                filename = dataset + "_kth_per_query_time_Top" + std::to_string(K) + "_threshold_" + std::to_string(CRACKTHRESHOLD) + ".csv";
                myfile.open (filename);
                myfile << "qid,Time," << endl;
                for(int i = 0; i < target_queries; i++) {
                    myfile << i << "," << per_query_time[i] << endl;
